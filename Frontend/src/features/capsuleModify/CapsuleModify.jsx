...
import { getCapsuleItems, getMyCapsuleInsurance } from '../capsule/api/capsuleInsurance.api';
...
const categories = [
    { id: 'silson', name: '실손 보험', color: 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200' },
    { id: 'disease', name: '질병 보험', color: 'bg-rose-100 text-rose-700 hover:bg-rose-200' },
    { id: 'liability', name: '생활 배상 보험', color: 'bg-blue-100 text-blue-700 hover:bg-blue-200' },
    { id: 'pet', name: '펫 보험', color: 'bg-amber-100 text-amber-700 hover:bg-amber-200' },
    { id: 'driver', name: '상시 운전자 보험', color: 'bg-purple-100 text-purple-700 hover:bg-purple-200' }
];
...
const CapsuleModify = () => {
    ...
    useEffect(() => {
        const fetchInit = async () => {
            try {
                ...
                setSelectedCells(data.selectedCells || Array.from({ length: data.targetAmount }, () => null));
            } catch (e) {
                console.error(e);
            }
        };
    ...
    useEffect(() => {
        ...
        if (amount > 0 && selectedCells.length !== amount) {
            ...
            if (newCells.length < amount) {
                // Add empty cells
                newCells = [...newCells, ...Array.from({ length: amount - newCells.length }, () => null)];
            } else {
    ...
    const handleAddItem = (item, cat) => {
        const firstEmptyIndex = selectedCells.findIndex(cell => cell === null);
    ...
    return (
        <div className="space-y-6 font-sans">
                ...
                <div className="animate-in fade-in slide-in-from-right-4 duration-500">
                    <div className="flex justify-between items-end border-b border-slate-200 pb-4 mb-6">
                        ...
                        <div className="flex items-center gap-3 bg-white px-4 py-2 rounded-xl border border-slate-200">
                            ...
                            />
                            <span className="font-bold text-slate-600">만원</span>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 min-h-[500px] flex flex-col">
                            <div className="bg-slate-100 rounded-2xl flex-1 border-2 border-dashed border-slate-300 p-8 grid grid-cols-5 gap-2 content-start">
                                {selectedCells.map((cell, i) => (
                                    <div
                                        ...
                                        className={`aspect-square sm:aspect-auto sm:h-20 rounded-xl border flex flex-col items-center justify-center transition-all ${cell ? `${cell.category.color} cursor-pointer hover:opacity-80` : 'bg-white/50 border-slate-200'
                                            }`}
                                    >
                                        {cell ? (
                            ...
                            <div className="mt-6 flex justify-between items-center">
                                <div className="flex items-center gap-2 text-sm text-slate-500 font-medium">
                                    <Info className="w-4 h-4" /> 탭하여 항목을 제거할 수 있습니다
                                </div>
                                <button
                        ...
                        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex flex-col h-[600px]">
                            <h3 className="font-bold text-slate-800 text-lg mb-4">보험 카테고리</h3>

                            <div className="flex items-center gap-2 mb-4 overflow-x-auto pb-2 scrollbar-hide">
                                <button onClick={() => setIsFilterModalOpen(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg text-sm font-bold transition-colors whitespace-nowrap">
                                    필터 커스텀
                                </button>
                                ...
                                {categories.map((cat) => (
                                    <div key={cat.id} className="border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                                        ...
                                        >
                                            ...
                                            {openCategory === cat.id ? <ChevronRight className="w-5 h-5 rotate-90 transition-transform" /> : <ChevronRight className="w-5 h-5 transition-transform" />}
                                        </button>
                                                ...
                                                {isLoadingItems ? (
                                                    ...
                                                    <div className="space-y-3">
                                                        {categoryItems.map((item) => (
                                                            <div key={item.id} className="group relative flex items-center justify-between p-3 bg-slate-50 hover:bg-slate-100 rounded-xl transition-colors cursor-pointer border border-transparent hover:border-slate-200" onClick={() => handleAddItem(item, cat)}>
                                                                <div className="flex flex-col">
                                                                    <span className="font-bold text-slate-700">{item.name}</span>
                                                                    <span className="text-xs text-slate-500 mt-1">{item.description}</span>
                                                                </div>
                                                                <div className="flex items-center gap-3">
                                                                    <span className="text-xs font-black px-2 py-1 bg-white rounded-md shadow-sm border border-slate-100 group-hover:bg-primary-50 group-hover:text-primary-600 transition-colors">1만원</span>
            ...
            {view === 'step-terms' && (
                <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200 animate-in fade-in slide-in-from-right-4">
                    <div className="mb-6 flex justify-between items-center">
                        <div>
                            <h2 className="text-2xl font-bold text-slate-800">예약 변경 약관 동의</h2>
                            <p className="text-slate-500 mt-1">새로 구성하신 보험의 약관을 확인해 주세요.</p>
                        </div>
                    </div>
                    {isSubmitting ? (
                        <div className="flex flex-col items-center justify-center py-20 space-y-4">
                            <Loader2 className="w-10 h-10 animate-spin text-primary-500" />
                            <p className="text-slate-600 font-medium text-lg">예약 정보를 저장 중입니다...</p>
                        </div>
                    ) : (
                        <TermsCheck
            ...
            <CustomModal isOpen={isSuccessModalOpen} onClose={handleSuccessClose}>
                <div className="text-center p-8 flex flex-col items-center">
                    <div className="w-20 h-20 bg-emerald-100 text-emerald-500 rounded-full flex items-center justify-center mb-6">
                        <CheckCircle className="w-10 h-10" />
                    </div>
                    <h3 className="text-2xl font-black text-slate-800 mb-2">예약 완료</h3>
                    <p className="text-slate-500 text-lg mb-8">
                        다음달 보험 구성 변경 예약이 완료되었습니다.<br />
            ...
            <CustomModal isOpen={isFilterModalOpen} onClose={() => setIsFilterModalOpen(false)}>
                <div className="text-center p-8">
                    <h3 className="text-xl font-bold text-slate-800 mb-2">필터 커스텀 모달</h3>
                    <p className="text-slate-500 text-sm">원하는 조건으로 보험 상품을 자세하게 필터링할 수 있는 모달입니다.</p>
                </div>
            </CustomModal>
        </div>
    );

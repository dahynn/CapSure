...
import { Plus, Info, ChevronRight, Loader2, ArrowRight } from 'lucide-react';
...
const CapsuleModify = () => {
    ...
    useEffect(() => {
        ...
        if (amount > 0 && selectedCells.length !== amount) {
            ...
            if (newCells.length < amount) {
                ...
                // Remove cells from the end, keeping filled ones if possible? 
                // As per basic requirement, just truncate.
    ...
    const handleSaveModify = () => {
        ...
        if (filled < Number(targetAmount)) {
            alert('목표 금액만큼 칸을 모투 채워주세요!');
            return;
        }
        alert('다음달 보험 구성이 예약되었습니다!');
    };
